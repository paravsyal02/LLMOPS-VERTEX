...
def extract_sentences_from_pdf(file):
    """Extracts sentences from an uploaded PDF file."""
...
def get_most_similar_sentence(query, sentences, embeddings):
    """Finds the most similar sentence from the PDF to the user's query."""
...
if uploaded_file is not None:
    ...
    if sentences:
        ...
        if embeddings:
            ...
            try:
                ...
                st.success("Embeddings and sentences saved locally.")
            except Exception as e:
                st.error(f"Error saving files locally: {e}")
                
            # Upload to GCS
            upload_to_gcs(bucket_name, "embeddings.json", embeddings)
            upload_to_gcs(bucket_name, "sentences.json", sentences)
            
            st.success("Embeddings and sentences uploaded to GCS.")
        else:
            st.error("No embeddings generated.")
    else:
        st.error("No sentences extracted from PDF.")
    ...
    if question:
        ...
        answer = get_most_similar_sentence(question, sentences, embeddings)
